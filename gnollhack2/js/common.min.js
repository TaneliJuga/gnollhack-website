...
modules.define('drop-menu', ['i-bem-dom', 'drop-menu__button'], function(provide, bemDom, button){
    let dMenu = bemDom.declBlock(this.name, {
...
modules.define('drop-menu__button', ['i-bem-dom'], function(provide, bemDom){
    var button = bemDom.declElem('drop-menu', 'button', {
        onSetMod: {
            'js': {
                'inited': function(){
                    this._button = this.domElem.not(this.__self._buildSelector('trigger', true));
                }
            }
        },
        _slide: function(fn, up){
            let time = this.__self.getSlideTime(this._button);
            if(up){
                this._emit(this.__self.slideUp);
                this._button.slideUp(time, fn);
            }else{
                this._emit(this.__self.slideDown);
                this._button.slideDown(time, fn);
            }
        },
        _slideUp: function(fn){
            let _this = this;
            function wrapper(){
                console.log("run wrapper");
                _this.delMod('checked');
                if(typeof fn === 'function'){
                    fn();
                }
            };

            let elem = this.findChildElem({
                elem: 'button',
                modName: 'checked',
                modVal: true
            });
            ...
            if(elem){
                function wrapperFunction(){
                    _this._slide(wrapper, true);
                }
                elem._slideUp(wrapperFunction);
            }else{
                this._slide(wrapper, true);   
            }
        },
        _slideDown: function(fn){
            let _this = this;
            wrapper = function(){
                _this.setMod('checked', true);
                if(typeof fn === 'function'){
                    fn();
                }
            };
        ...
        _toggleSlide: function(){
            if(this.hasMod('checked')){
                ...
                let current = this._block()._current;
                this._block()._current = this;
                let _this = this;
                if(current && current !== this){
                    current._slideUp(function(){
                        _this._slideDown();
                    });
                }else{
                    this._slideDown();
    ...
    },
    {
        ...
        slideSpeed: 0.2,
        getSlideTime: function(node){
            return node.height() * (1 / this.slideSpeed);
        },
        lazyInit: true
    });
...
provide(bemDom.declBlock(this.name,{
    ...
    onInit : function() {
        console.log(this);
        this._domEvents().on('click', function(){alert("pointer press")});
        return this.__base.apply(this, arguments);
    }
}));
...
modules.define('bem-table', ['i-bem-dom', 'bem-table__search', 'bem-table__row', 'bem-table__head', 'jquery'], function(provide, bemDom, Search, Row, Head, $){
    provide(bemDom.declBlock(this.name, 
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        this._table = this.findChildElem('table');
                        this._searchColumn = this.getColumn(0);

                        const bodyElem = this._tBody = this.findChildElem('body').domElem;
                        this._heightOffset = this.domElem.height() - bodyElem.height();
                    }
                }
            },
            _onSearch: function(e, data){
                var value, column, cellHeight = this._heightOffset;
                column = this._searchColumn;
                value = data.toUpperCase();
                //this._tBody.height(this._tBody.height);
                column.each(function(){
                    const show = $(this).text().toUpperCase().indexOf(value) > -1;
                ...
                this.domElem.animate({height: cellHeight}, 500);
            },
            _onSort: function(e){
                const head = e.bemTarget, 
                index = head.getIndex();

                sortTable(this._table.domElem[0], index);
            },
            getColumn: function(i){
                return $(this.findChildElems(Row).map(row => {
        ...
        }, 
        {
            ...
            onInit: function(){
                ...
                this._events(Search).on('search', ptp._onSearch);
                this._events(Head).on('sort', ptp._onSort);
            }
        })
    );

    function sortTable(table, n){
        var table, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0, n, tableHeaders;
        switching = true;
        rows = table.rows;
        tableHeaders = rows[0].children;
...
modules.define('drop-menu__caret', ['i-bem-dom', 'drop-menu__button'], function(provide, bemDom, Button){
    provide(bemDom.declElem('drop-menu', 'caret', 
        {
            // onSetMod: {
            //     'js': {
            //         'inited': function(){
            ...
            //     },
            // },
            _onSlideUp: function(e){
                // this.delMod('up');
                // //e.stopPropagation();
                this._elem('caret').delMod('up');
            },
            _onSlideDown: function(e){
                // this.setMod('up', true);
                // //e.stopPropagation();
                this._elem('caret').setMod('up', true);
            }
            // ,
            // _init: function(button, slideUp, slideDown){
            //     this._events(button).on(slideUp, this._onSlideUp);
            //     this._events(button).on(slideDown, this._onSlideDown);
            // }
        },
        {
            lazyInit: true,
            onInit: function(){
                Button._events().on(Button.slideDown, this.prototype._onSlideDown);
                Button._events().on(Button.slideUp, this.prototype._onSlideUp);
            }
        },
        ));
});
modules.define('drop-menu__button', [], function(provide, Button){
    provide(Button.declMod({modName: 'trigger'}, 
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        this.__base.apply(this, arguments);
                        // let cls = this.__self;
                        // this.findChildElem(Caret)._init(this, cls.slideUp, cls.slideDown);
                    }
                }
            }
        },
...
modules.define('top-nav__item', ['i-bem-dom', 'drop-menu', 'drop-menu__button'], function(provide, bemDom, dropMenu, Button){
    let itemToButton = new Map();
    function getButton(item){
        return itemToButton.get(item);
    }

    let item = bemDom.declElem('top-nav', 'item', 
        {
            _onDropMenuOpen: function(e){
                this.setMod('glow', true);
                console.log("this.setMod('glow', true);");
            },
            _onDropMenuClose: function(e){
                this.delMod('glow');
                console.log("this.delMod('glow');");
                console.log(this);
                console.log(e);
            },
            _init: function(e){
                let dropMenu = e.bemTarget();
                this._events(dropMenu).on(Button.slideDown, this._onDropMenuOpen);
                this._events(dropMenu).on(Button.slideUp, this._onDropMenuClose);
            },
            _setButton: function(button){
                if(button instanceof Button){
                    itemToButton.set(this, button);
                    return true;
                }
                return false;
            }
        },
        {
            lazyInit: true,
            onInit: function(){
                // let wrapperFn = function(e){
                //     this._init(e);
                //     this._onDropMenuClose(e);
                // }
                 console.log(Button.prototype);
                 console.log(dropMenu.prototype);
                // console.log(Button._block);
                // console.log(Button.getName());
                // console.log(Button.getEntityName());
                // console.log(Button.__parent);
                // console.log(Button.__parent.getName());
                // console.log(Button.__parent.getEntityName());
                //this._events(Button.__parent).once(Button.slideDown, wrapperFn);
                let topNavItemCls = this;
                // function init(e){
                //     let item = this.findParentElem(topNavItemCls);
                //     if(item){
                //         //item._setButton(this);
                //         this._events().on(Button.slideDown, item._onDropMenuOpen, item);
                //         this._events().on({modName: 'checked', modVal: ''}, item._onDropMenuClose, item);
                //         item._onDropMenuOpen(e);
                //     }
                // }
                function init(e){
                    let item = this.findParentElem(topNavItemCls);
                    ...
                    if(item){
                        ...
                        item._button = this;
                        item._events(this).on(Button.slideDown, makeWrapper(item._onDropMenuOpen));
                        item._events(this).on({modName: 'checked', modVal: ''}, makeWrapper(item._onDropMenuClose));
                        item._onDropMenuOpen(e);
                    }
                }
                ...
                //Button._events().on(Button.slideDown, this.prototype._onDropMenuClose);
                //Button._events().on({modName: 'checked', modVal: ''}, stopEventPropagation);
                Button._events().once(Button.slideDown, init);
                // function stopEventPropagation(e){
                //     e.stopPropagation();
                // }
                //Button._events().on(Button.slideDown, stopEventPropagation);
                //this._events().once(Button.slideUp, init);
            }
        }   
    );
...
modules.define('bem-table__head', ['i-bem-dom'], function(provide, bemDom){
    provide(bemDom.declElem('bem-table', 'head', 
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        this._index = this.domElem.index();
                    }
                }
            },
            getIndex: function(){
...
modules.define('bem-table__t-head', 
['i-bem-dom', 'checkbox-group', 'checkbox', 'bem-table__head'], 
function(provide, bemDom, CheckboxGroup, Checkbox, Head){
    function State(val, dir, nextState){
        this.val = val;
        this.dir = dir;
        this.nextState = nextState;
    }
    const checked = new State(true, 'ascending');
    const unchecked = new State('', 'descending', checked);
    checked.nextState = unchecked;
    function StateMachine(states){
        this.getState = function(modVal){
            return states.find(state => {
                return state.val === modVal;
            });
        },
        this.getDirection = function(modVal){
            return this.getState(modVal).nextState.dir;
        }
    }
    const checkboxStateMachine = new StateMachine([checked, unchecked]);
    
    provide(bemDom.declElem('bem-table', 't-head',
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        const checkboxGroup = this._checkboxGroup = this.findChildBlock(CheckboxGroup);
                        this._activeCount = 0;
                        checkboxGroup.getCheckboxes().forEach(e => {
                            setCheckboxTitle(e);
                        });

                        this.__self._events(Checkbox).on({modName: 'checked', modVal: '*'}, this._checkboxTitleHandler);
                        this._setCheckboxCheckedHandler(this._onCheckboxChecked, true);
                        this._setCheckboxCheckedHandler(this._onCheckboxUnchecked, '');
                    }
                }
            },
            _setCheckboxCheckedHandler: function(fn, modVal){
                _this = this;
                const handler = function(e, data){
                    fn.call(_this, e.bemTarget, this, data);
                }
                Head._events(Checkbox).on({modName: 'checked', modVal: modVal}, handler);
            },
            _onCheckboxChecked: function(checkbox, head){
                const checkboxVal = checkbox.getVal();
                this._activeCount++;

                this._checkboxGroup.setVal([checkboxVal]);
                head.sort();
            },
            _onCheckboxUnchecked: function(checkbox, head){
                this._activeCount--;
            ...
            _checkboxTitleHandler: function(e, data){
                const checkbox = e.bemTarget;

                setCheckboxTitle(checkbox, data.modVal);
            }
        },
        {
    ...
    function setCheckboxTitle(checkbox, modVal){
        modVal || (modVal = checkbox.getMod()); 
        
        const dir = checkboxStateMachine.getDirection(modVal),
            title = 'Sort ' + dir;

        checkbox.domElem.attr('title', title);
    }


});
modules.define('caret', ['i-bem-dom'], function(provide, bemDom){
    provide(bemDom.declBlock(this.name, 
        {
            _onClick: function(e){
                this.toggleMod('up');
...
modules.define('checkbox', ['caret'], function(provide, Caret, Checkbox){
    const props = [
        [true, true, 'checked', 'up'],
        [false, true, 'disabled'],
        [true, false, 'focused']
    ].map(e => {
        return {
    ...
    provide(Checkbox.declMod({modName: 'type', modVal: 'caret'},
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        this.__base.apply(this, arguments);
                        const caret = this._caret = this.findChildBlock(Caret);
                        const _this = this;

                        props.forEach(e => {
                            registerHandler(this, e.checkboxMod, makeHandler(proxyModForCaret));
                
                            if(e.isTwoWayBind){
                                registerHandler(caret, e.caretMod, makeHandler(proxyModFromCaret));
                            }

                            function makeHandler(proxyFn){
                                return function(event, data){
                                    const modName = data.modName, 
                                        modVal = data.modVal;
                                    proxyFn.call(this, modName, modVal, e.callBase);
                                }
                            }
                        });
                
                        function registerHandler(instance, modName, handler){
                            instance._events().on(
                                {modName: modName, modVal: '*'},
                                handler,
                                _this
                            );
                        }
    ...
    function proxyModForCaret(modName, modVal, callBase){
        //callBase !== false && this.__base.apply(this, arguments);
        this._caret.setMod(findMod(modName, 'checkboxMod'), modVal);
    }

    function proxyModFromCaret(modName, modVal){
        this.setMod(findMod(modName, 'caretMod'), modVal);
    }

    function findMod(modName, propName){
        const otherProp = propName === 'caretMod' ? 'checkboxMod' : 'caretMod';

        return props.find(e => {
            return e[propName] === modName
        })[otherProp];
    }
});
modules.define('bem-table__search', ['i-bem-dom', 'input'], function(provide, bemDom, Input){
    provide(bemDom.declElem('bem-table', 'search', 
        {
            onSetMod: {
                'js': {
                    'inited': function(){
                        this._input = this.findMixedBlock(Input);
                    }
                }
            },
            _onChange: function(e){
                this._emit('search', this._input.getVal());
            }
        }, 
        {
            lazyInit: true,
            onInit: function(){
                this._events(Input).on('change', this.prototype._onChange);
            }
        }
    ));
});
...
modules.define('bem-table__row', ['i-bem-dom'], function(provide, bemDom){
    provide(bemDom.declElem('bem-table', 'row',
        {
            getCell: function(index){
                return this.domElem.children().eq(index);
            }
        }, 
        {
            lazyInit: true
